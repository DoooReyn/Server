...
struct ShareObject
{
	ShareObject()	{ m_status = SHARED_NONE, m_nFlag = 0xFE; }
	bool IsModify() { return m_status == SHARED_MODIFY; }
	bool IsDelete() { return m_status == SHARED_DELETE; }
	void Modify()	{ m_status = SHARED_MODIFY; }
	void Destroy()  { m_status = SHARED_DELETE;}
	void Reset()	{ m_status = SHARED_NONE; }
...
public:
	...
	uint32 GetCount()		{ return m_pageNum * m_count; };
	uint32 GetPage()		{ return m_pageNum; };
	uint32 GetFreeCount()	{ return m_mapFreeSMBlock.size(); }
	uint32 GetUsedCount()	{ return m_mapUsedSMBlock.size(); }
...
public:
	...
	T* NewObject(bool isNew = false)
	{
		...
		if (pTmp == NULL)
		{
			printf("NewObject is return NULL \n");
		}
